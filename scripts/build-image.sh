...
if [ -n "${PLATFORMS:-}" ]; then
  ...
  docker buildx build --platform "$PLATFORMS" "$OUTPUT_FLAG" --build-arg "APP_VERSION=$APP_VERSION_VALUE" -t "$IMAGE_REF" .
else
  docker build --pull --build-arg "APP_VERSION=$APP_VERSION_VALUE" -t "$IMAGE_REF" .
fi
